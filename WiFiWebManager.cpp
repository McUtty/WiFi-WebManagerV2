...
String WifiWebManager::renderMenu(const String& currentPath) {
    ...
    // Standardseiten zuerst
    PageEntry stdpages[] = {
        {"WLAN", "/"}, {"NTP", "/ntp"}, {"Firmware", "/update"}, {"Reset", "/reset"}
    };
    for (auto &p : stdpages) {
        nav += "<a href='" + p.path + "'";
...
String WifiWebManager::htmlWrap(const String& menutitle, const String& currentPath, const String& content) {
    String css = R"rawliteral(
      <style>body{background:#f3f6fa;font-family:sans-serif;margin:0;}
      ...
      button:hover,input[type=submit]:hover{background:#1064b0;}nav{width:100%;margin-bottom:1.5em;display:flex;
      justify-content:center;gap:1em;}nav a{text-decoration:none;color:#2584fc;font-weight:600;font-size:1.1em;
      padding-bottom:2px;border-bottom:2px solid transparent;transition:border-color 0.2s;}
      nav a.selected,nav a:hover{border-color:#2584fc;}@media (max-width:600px){.centerbox{max-width:99vw;padding:1em;}
...
void WifiWebManager::setupWebServer() {
    ...
    server.on("/", HTTP_GET, [this](AsyncWebServerRequest *request){
        ...
        html += "> Statische IP aktivieren</label>"
            ...
            "</form>";
        request->send(200, "text/html", htmlWrap("ESP32 WebKonfig", "/", html));
    });
    ...
    server.on("/save", HTTP_POST, [this](AsyncWebServerRequest *request){
        ...
        shouldReboot = true;
        request->send(200, "text/html", htmlWrap("Speichern", "/", "<p>Gespeichert! Neustart...</p><a href='/'>Zurück</a>"));
    });
    ...
    server.on("/reset", HTTP_GET, [this](AsyncWebServerRequest *request){
        ...
        request->send(200, "text/html", htmlWrap("Reset", "/reset", html));
    });
    server.on("/reset", HTTP_POST, [this](AsyncWebServerRequest *request){
        clearAllConfig();
        shouldReboot = true;
        request->send(200, "text/html", htmlWrap("Reset", "/reset", "<p>Werksreset ausgeführt. Neustart...</p>"));
    });
    ...
    server.on("/update", HTTP_GET, [this](AsyncWebServerRequest *request){
        ...
        request->send(200, "text/html", htmlWrap("Firmware", "/update", html));
    });
    ...
    server.on("/ntp", HTTP_GET, [this](AsyncWebServerRequest *request){
        ...
        request->send(200, "text/html", htmlWrap("NTP Einstellungen", "/ntp", html));
    });
    server.on("/ntp_save", HTTP_POST, [this](AsyncWebServerRequest *request){
        ...
        request->send(200, "text/html", htmlWrap("NTP Einstellungen", "/ntp", "<p>NTP gespeichert!</p><a href='/ntp'>Zurück</a>"));
    });
